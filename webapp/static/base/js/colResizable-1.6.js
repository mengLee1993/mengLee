...
(function($){ 	
	...
	var init = function( tb, options){	
		...
		//LIMENG注释和更换
        // if(!t.is("table") && !t.opt.partialRefresh) return;
        if(!t.is("table") || tables[id] && !t.opt.partialRefresh)
        	return; 		//if the object is not a table or if it was already processed then it is ignored.
	...
	*/
	...
	var onGripDragOver = function(e){	
        ...
        if(typeof(onGripDragOver$after)=="function")
            onGripDragOver$after(drag.c[0].cellIndex,drag.c.w,drag.t[0]);
...
//mouseUp之后的事件 1。找到第几个元素   2。宽度  3。table表格Dom
// function onGripDragOver$after(tdIndex,tdWidth,tableDom){
//     // $(tableDom).find("tr td").eq(tdIndex).find(".chosen-container").addClass("w")
// }
