...
/*-------------------------- +
  ÍÏ×§º¯Êý
 +-------------------------- */
function drag(oDrag, handle,isTips,formId)
{
	var disX = dixY = 0;
	...
	handle.onmousedown = function (event)
	{
		...
		{
			//hideAllErrInfo(document.getElementById(formId));
			var event = event || window.event;
...
function openWin(nW,nH,id,bol,isTips,formId)
{
	...
	drag(oDrag, oTitle,isTips,formId);
	//ËÄ½Ç
	...
	oDrag.style.left = ($('#'+id).parent().width() - oDrag.offsetWidth) / 2 + "px";
	...
	//oDrag.style.top=scrollTopVal+window.screen.height/2-(oDrag.offsetHeight)/2+'px';
	oDrag.style.top=scrollTopVal+window.screen.height/2-(oDrag.offsetHeight)/2 - 100 +'px';
	oDrag.style.left=($('#'+id).parent().width()- oDrag.offsetWidth)/2+'px';
	currPopDiv = oDrag;
