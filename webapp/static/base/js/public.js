...
/**
 * 文件上传
 \ennew-c2p-webapps-wwwp\src\main\webapp\sdapstatic\uploadcompressFile.html
 \ennew-c2p-webapps-wwwp\src\main\webapp\sdapstatic\uploadFile.html
 ennew-c2p-webapps-wwwp\src\main\webapp\sdapstatic\uploadCutFile.html
...
function getCurrentDate() {
    ...
    if (new Date().getMonth() + 1 < 10 && new Date().getDate() < 10) {
        return obj.getFullYear() + "-0" + (new Date().getMonth() + 1) + "-0" + new Date().getDate();
    } else if (new Date().getMonth() + 1 < 10 && new Date().getDate() >= 10) {
        return obj.getFullYear() + "-0" + (new Date().getMonth() + 1) + "-" + new Date().getDate();
    } else if (new Date().getMonth() + 1 >= 10 && new Date().getDate() < 10) {
        return obj.getFullYear() + "-" + (new Date().getMonth() + 1) + "-0" + new Date().getDate();
    } else {
        return obj.getFullYear() + "-" + (new Date().getMonth() + 1) + "-" + new Date().getDate();
    }
}
jQuery.download = function (url, data, method, dom) { // 获得url和data dom当前点击的对象
    if (dom) {//传入dom会有倒计时操作
        //如果没有timeWait属性，默认3s
        if (dom.getAttribute("timeWait") == null)
            dom.setAttribute("timeWait", 3);
        //动态创建倒计时标签
        var timeWait = dom.getAttribute("timeWait");
        $(dom).attr("disabled", true).addClass("changeGary");
        $(dom).append("<span style='display: inline;' id='timeWait'>(<span style='display: inline;'>" + timeWait + "</span>s)</span>")
        //计时器
        var int = setInterval(function () {

            if (timeWait == 0) {
                //销毁计时器 删除创建的倒计时标签
                $(dom).find("#timeWait").remove();
                $(dom).attr("disabled", false).removeClass("changeGary");
                clearInterval(int);
            } else {
                timeWait = timeWait - 1;
                $(dom).find("#timeWait span").html(timeWait);
            }
        }, 1000);
    }
    if (url && data) {
...
function CurentTime(prev, times) {//prev    prev前一个月 next下一个月
    var now = new Date();
    if (times) {
        now = new Date(times);
    }
...
function fixationTable(scrollDivId){
	nayiLock(scrollDivId);
}
...
//              "background-color": "#daefff",
                "z-index": jQuery(this).hasClass("lock-row") ? 20 : 10
            });
        });
        ...
        jQuery(this).find(".lock-row").each(function () {
            jQuery(this).css({
                ...
                "top": top,
//              "background-color": "#daefff",
                "z-index": jQuery(this).hasClass("lock-col") ? 20 : 9
            });
        });
    });
}
/*json去重*/
function removeDuplicate(jsonData, jsonNewData) {
    for (key in jsonData) {
        if (jsonNewData[key] == jsonData[key] || jsonNewData[key] == null) {
            delete jsonNewData[key];
        }

        else {
            if (typeof (jsonData[key]) == "object") {
                if ((jsonData[key] == null && jsonNewData[key] == null) || (jsonData[key] == "" && jsonNewData[key] == null) || (jsonData[key] == null && jsonNewData[key] == ""))
                    delete jsonNewData[key];
                else {
                    // removeDuplicate(jsonData[key],jsonNewData[key]);
                    jsonNewData[key + "New"] = jsonNewData[key];
                    jsonNewData[key] = jsonData[key];
                }
...
function limitCodeDeal(domArr,attrName){//codeArr：请求后台，返回code集合。||domArr：获取页面集所有带有limitCode的节点。|| attrName: 页面内code码的自定义属性
    ...
    if(window.Storage && window.sessionStorage && window.sessionStorage instanceof Storage){
        var codeArr = window.sessionStorage[attrName] ? JSON.parse(window.sessionStorage[attrName]) : [];
    }else{
    ...
    for(var nI = 0; nI < domArr.length; nI++ ){
        ...
        if(isTrue){//说明domArr集合中没有匹配到后台返回的codeArr集合中的权限整理，则此dom节点没有权限，进行remove操作
            domArr.eq(nI).css("display","block");
        }
		else
			domArr.eq(nI).css("display","none");
    }
...
//limitCodeDeal("limitCodeBtn");
//增加打印公共(加工)方法
var printPublicFun = (function () {

    var _bind = function (that) {
        //打印
        that.printBtn.onclick = function () {
            var printSid = that.getTableJsonData();
            if (!that.flag) {
                //暂时做单个打印
                if (!printSid.length || printSid.length > 1) {
                    var alertBox = new clsAlertBoxCtrl();
                    alertBox.Alert("请勾选一条要打印的数据", "提示");
                    return false;
                }
            } else {
                if (!printSid.length) {
                    var alertBox = new clsAlertBoxCtrl();
                    alertBox.Alert("请最少一条要打印的数据", "提示");
                    return false;
                }
            }
            printSid = printSid.join(",");
            var companyCode = null;
            if (getCookieData) {
                companyCode = getCookieData.companyCode;
            }
            if (!that.ajax) {
                jumpUrl(printDomain + that.url + printSid + "&companyCode=" + companyCode + "&scroll=yes&scale=1.5", "0000000", 1);
            } else {
                var params = { "reportCode": that.ajax };
                getAjaxResult("/md-report-config/findUniqueReportConfigByParams", "post", params, function (data) {
                    var json = JSON.parse(data);
                    if (json.retCode == "0000000") {
                        if(json.rspBody){
                            jumpUrl(printDomain + "/showReport.jsp?rpx=/process/" + json.rspBody.reportFileName + that.url + printSid + "&companyCode=" + getCookieData.companyCode + "&scroll=yes&scale=1.5", "0000000", 1);
                        }
                    }
                })
    ...
    printFun.prototype.init = function (config) {
        this.tableId = document.getElementById(config.tableId);
        this.printBtn = document.getElementById(config.printBtn);
        this.sid = config.sid;
        this.url = config.url;
        this.flag = config.flag ? config.flag : 0
        this.ajax = config.ajax ? config.ajax : 0
        _bind(this);
        return this;
    }
    //获取参数
    printFun.prototype.getTableJsonData = function () {
        var arr = [];
        var that = this;
        $(this.tableId).find("tr[id='cloneRow']").each(function (i, item) {
            var jsonData = item.jsonData;
            if ($(item).find(".clsChk").prop("checked")) {
                arr.push(jsonData[that.sid])
            }
        })
        return arr;
    }
