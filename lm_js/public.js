...
/**
 *
 * @param oText  需要计数的input或者textarea
 * @param oNum   动态变化的数字标签
 * @param nLen   长度
 */
function hasWriteinWordNum(oText, oNum, nLen) {
    var nLen = (nLen == null) ? 10 : nLen;
    $(oText).on('keyup', function () {
        // $(oText).val(this.value.replace(/\s/g, ""));
        if (this.value.length >= nLen) {
            this.value = this.value.substring(0, nLen);
            oNum && $(oNum).html(nLen);
        } else {
            oNum && $(oNum).html($(this).val().length);
        }
...
/**
 * 购买数量控制组件
 * options true--数量可以为小数  null/false--整数；num 小数位数
 * 调用代码$("selector").countIpt()  整数
 * 调用代码$("selector").countIpt(true,2)  可输入两位小数
 *
 */
(function ($) {
    $.fn.countIpt=function(options,num){
        var $this=this;
        var min = this.find('a')[0];
        var add = this.find('a')[1];
        var i = this.find('input')[0].value;
        ...
        input.onblur = function(){
            countIptBlurCallBack(this);
        }
        min.onclick = function () {
            options?i<1?i=0:i--:i<=1?i=1:i--;
            options?input.value=i.toFixed(num):input.value=i;
            countIptCallBack(0,this);
        };
        add.onclick = function () {
            i++;options?input.value=i.toFixed(num):input.value=i;
            countIptCallBack(1,this);
        }
    }
})(jQuery)

/**
 * optType 0 jian    1 jia
 * @param optType
 */
function countIptCallBack(optType){
 ...
 */
function countIptBlurCallBack(inputDom){

}

//cookie读取缓存操作
//获取cookie name为cookie中的key jsonHead为头部传参json
function getCookie(c_name,jsonHead)
{
    /*//域名区分
    var hrefUrl = document.location.href.split("/")[2];
    if(hrefUrl == ""){
        //渠道前台  平台前台 万象wxsid（万象登录系统）
        // 渠道后台qhsid（渠道后台登录系统）
        // 平台后台phsid（平台后台登录系统）
        // 网格超市wgsid（网格超市登录系统）
        c_name = "wx" + c_name;
    ...
    if (document.cookie.length>0)
    {
        ...
        if (c_start!=-1)
        {
            ...
            jsonHead.sid = unescape(document.cookie.substring(c_start,c_end));
...
//存储cookie
function setCookie(c_name,value,jsonHead)
{
    /*//域名区分
    var hrefUrl = document.location.href.split("/")[2];
    if(hrefUrl == ""){
...
//     });
    ...
    if(c_name == "sid")
        jsonHead.sid = unescape(value);
    if(c_name == "channelId")
        jsonHead.channelId = unescape(value);
}
//删除cookie
...
//关于阳光易购应用间的operCode操作，三级权限
function operPermission(that){
    if(that.attr("operPermission") == 0){
        alert("此用户无操作权限");
        return false;
    }else{
        return true;
...
function operCodeThirdCallBack(data){//三级权限接口回调函数
    data = JSON.parse(data);
    if(data.retCode == "0000000"){
        //功能code数组
        var dataRspFunctionCode = data.rspBody.functionCode;
        //操作code数组
        var dataRspOperCode = data.rspBody.operCode;
        $("*[functionCode]").each(function(){
            //全部设置无操作权限
            $(this).attr("operPermission","0");
            if($(this).attr("specialCode") == 1){//此标识为是否具有单独点击事件，如果为插件按钮，没有点击事件，则直接隐藏
                $(this).hide();
            }
            //循环接口返回  是否具有权限
            for(var nI = 0; nI < dataRspFunctionCode.length; nI++ ){
                if($(this).attr("functionCode") == dataRspFunctionCode[nI]){//有权限操作
                    $(this).attr("operPermission","1");
                    if($(this).attr("specialCode") == 1){
                        $(this).show();
                    }
                    return;
                }
        ...
        $("*[operCode]").each(function(){
            $(this).attr("operPermission","0");
            if($(this).attr("specialCode") == 1){//此标识为是否具有单独点击事件，如果为插件按钮，没有点击事件，则直接隐藏
                $(this).hide();
            }
            for(var nI = 0; nI < dataRspOperCode.length; nI++ ){
                if($(this).attr("operCode") == dataRspOperCode[nI]){//有权限操作
                    $(this).attr("operPermission","1");
                    if($(this).attr("specialCode") == 1){
                        $(this).show();
                    }
